...
func (r1cs *UntypedR1CS) toBW761() *backend_bw761.R1CS {
	...
	for i := 0; i < len(r1cs.Constraints); i++ {
		...
		for j := 0; j < len(from.L); j++ {
			...
			to.L[j] = backend_bw761.NewTerm(int(from.L[j].ID), cID, specialValue)
		}
		for j := 0; j < len(from.R); j++ {
			cID, specialValue = getCoeffIdx(&from.R[j].Coeff)
			to.R[j] = backend_bw761.NewTerm(int(from.R[j].ID), cID, specialValue)
		}
		for j := 0; j < len(from.O); j++ {
			cID, specialValue = getCoeffIdx(&from.O[j].Coeff)
			to.O[j] = backend_bw761.NewTerm(int(from.O[j].ID), cID, specialValue)
		}
