...
func TestCircuits(t *testing.T) {
	...
	for _, name := range matches {
		...
		r1cs := fr1cs.ToR1CS(curve.ID).(*backend_bls377.R1CS)
...
func TestParsePublicInput(t *testing.T) {
	...
	if _, err := groth16_bls377.ParsePublicInput(expectedNames[:], inputOneWire); err == nil {
		t.Fatal("expected ErrMissingAssigment error")
	}

	missingInput := make(map[string]interface{})
	if _, err := groth16_bls377.ParsePublicInput(expectedNames[:], missingInput); err == nil {
		t.Fatal("expected ErrMissingAssigment")
	}

	correctInput := make(map[string]interface{})
	correctInput["data"] = 3
	got, err := groth16_bls377.ParsePublicInput(expectedNames[:], correctInput)
	if err != nil {
		t.Fatal(err)
	}
...
func referenceCircuit() (backend_bls377.R1CS, map[string]interface{}, map[string]interface{}) {
...
func BenchmarkProver(b *testing.B) {
	...
	b.Run("prover", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			_, _ = groth16_bls377.Prove(&r1cs, &pk, solution)
		}
	})
}
...
func BenchmarkVerifier(b *testing.B) {
	...
	var vk groth16_bls377.VerifyingKey
	groth16_bls377.Setup(&r1cs, &pk, &vk)
	proof, err := groth16_bls377.Prove(&r1cs, &pk, solution)
	if err != nil {
		panic(err)
	}
	...
	b.Run("verifier", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			_, _ = groth16_bls377.Verify(proof, &vk, solution)
		}
	})
}
