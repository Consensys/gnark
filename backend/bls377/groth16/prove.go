...
func Prove(r1cs *backend_bls377.R1CS, pk *ProvingKey, solution backend.Assignments) (*Proof, error) {
	...
	// fft domain (computeH)
	fftDomain := backend_bls377.NewDomain(root, MaxOrder, r1cs.NbConstraints)
...
func computeKrs(pk *ProvingKey, r, s, _r, _s fr.Element, wireValues []fr.Element, ar, bs curve.G1Affine, h []fr.Element, kIndex int, chToken chan struct{}) <-chan curve.G1Affine {
	...
	go func() {
		...
		<-chToken
		chAsync := Krs.MultiExp(curve.BLS377(), points, scalars)
		<-chAsync
...
func computeBs2(pk *ProvingKey, _s fr.Element, wireValues []fr.Element, chToken chan struct{}) <-chan curve.G2Affine {
	...
	go func() {
		...
		<-chToken
		chAsync := Bs.MultiExp(curve.BLS377(), points2, scalars2)
		chToken <- struct{}{}
		<-chAsync
...
func computeBs1(pk *ProvingKey, _s fr.Element, wireValues []fr.Element, chTokenA, chTokenB chan struct{}) <-chan curve.G1Affine {
	...
	go func() {
		...
		<-chTokenA
		chAsync := bs1.MultiExp(curve.BLS377(), points, scalars)
		chTokenB <- struct{}{}
		<-chAsync
...
func computeAr1(pk *ProvingKey, _r fr.Element, wireValues []fr.Element, chToken chan struct{}) <-chan curve.G1Affine {
	...
	go func() {
		...
		chAsync := ar.MultiExp(curve.BLS377(), points, scalars)
		chToken <- struct{}{}
		<-chAsync
...
func computeH(a, b, c []fr.Element, fftDomain *backend_bls377.Domain) <-chan []fr.Element {
