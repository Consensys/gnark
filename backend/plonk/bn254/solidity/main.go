...
func main() {
	...
	err = bn254plonk.Verify(&proof, &vk, pi)
	checkError(err)
	...
	for _, vLog := range logs {
		var event interface{}
		err = contractABI.UnpackIntoInterface(&event, "PrintSuccess", vLog.Data)
		checkError(err)
		fmt.Println(event)
	}
}
