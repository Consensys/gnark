...
contract PlonkVerifier {
  ...
  internal view returns (uint256 res) {
    assembly {
      ...
      // _n^_i [r]
      function pow_local(x, e, mPtr)->result {
          // let mPtr := mload(0x40)
          mstore(mPtr, 0x20)
      ...
      let w := pow_local(vk_omega, i, freeMem) // w**i
      i := addmod(zeta, sub(r_mod, w), r_mod) // z-w**i
      w := mulmod(w, vk_inv_domain_size, r_mod) // w**i/n
      i := pow_local(i, sub(r_mod,2), freeMem) // (z-w**i)**-1
  ...
  function compute_pi(
    ...
    assembly {
      ...
      function sum_pi_commit(aproof, nb_public_inputs, z, zpnmo, mPtr)->pi_commit {
      ...
      // res <- x^e mod r, mPtr = free memory
