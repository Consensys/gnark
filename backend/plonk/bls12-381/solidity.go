...
contract PlonkVerifier {
  ...
  uint256 private constant G2_SRS_{{ $index }}_X_0_lo = {{ (fpstr_lo $element.X.A1) }};
	uint256 private constant G2_SRS_{{ $index }}_X_0_hi = {{ (fpstr_hi $element.X.A1) }};
  uint256 private constant G2_SRS_{{ $index }}_X_1_lo = {{ (fpstr_lo $element.X.A0) }};
  uint256 private constant G2_SRS_{{ $index }}_X_1_hi = {{ (fpstr_hi $element.X.A0) }};
  uint256 private constant G2_SRS_{{ $index }}_Y_0_lo = {{ (fpstr_lo $element.Y.A1) }};
  uint256 private constant G2_SRS_{{ $index }}_Y_0_hi = {{ (fpstr_hi $element.Y.A1) }};
  uint256 private constant G2_SRS_{{ $index }}_Y_1_lo = {{ (fpstr_lo $element.Y.A0) }};
  uint256 private constant G2_SRS_{{ $index }}_Y_1_hi = {{ (fpstr_hi $element.Y.A0) }};
  {{ end }}
  uint256 private constant G1_SRS_X_lo = {{ fpstr_lo .Vk.Kzg.G1.X }};
	uint256 private constant G1_SRS_X_hi = {{ fpstr_hi .Vk.Kzg.G1.X }};
  uint256 private constant G1_SRS_Y_lo = {{ fpstr_lo .Vk.Kzg.G1.Y }};
  uint256 private constant G1_SRS_Y_hi = {{ fpstr_hi .Vk.Kzg.G1.Y }};
  ...
  uint256 private constant VK_QL_COM_X_lo = {{ (fpstr_lo .Vk.Ql.X) }};
	uint256 private constant VK_QL_COM_X_hi = {{ (fpstr_hi .Vk.Ql.X) }};
  uint256 private constant VK_QL_COM_Y_lo = {{ (fpstr_lo .Vk.Ql.Y) }};
  uint256 private constant VK_QL_COM_Y_hi = {{ (fpstr_hi .Vk.Ql.Y) }};
  uint256 private constant VK_QR_COM_X_lo = {{ (fpstr_lo .Vk.Qr.X) }};
  uint256 private constant VK_QR_COM_X_hi = {{ (fpstr_hi .Vk.Qr.X) }};
  uint256 private constant VK_QR_COM_Y_lo = {{ (fpstr_lo .Vk.Qr.Y) }};
  uint256 private constant VK_QR_COM_Y_hi = {{ (fpstr_hi .Vk.Qr.Y) }};
  uint256 private constant VK_QM_COM_X_lo = {{ (fpstr_lo .Vk.Qm.X) }};
  uint256 private constant VK_QM_COM_X_hi = {{ (fpstr_hi .Vk.Qm.X) }};
  uint256 private constant VK_QM_COM_Y_lo = {{ (fpstr_lo .Vk.Qm.Y) }};
  uint256 private constant VK_QM_COM_Y_hi = {{ (fpstr_hi .Vk.Qm.Y) }};
  uint256 private constant VK_QO_COM_X_lo = {{ (fpstr_lo .Vk.Qo.X) }};
  uint256 private constant VK_QO_COM_X_hi = {{ (fpstr_hi .Vk.Qo.X) }};
  uint256 private constant VK_QO_COM_Y_lo = {{ (fpstr_lo .Vk.Qo.Y) }};
  uint256 private constant VK_QO_COM_Y_hi = {{ (fpstr_hi .Vk.Qo.Y) }};
  uint256 private constant VK_QK_COM_X_lo = {{ (fpstr_lo .Vk.Qk.X) }};
  uint256 private constant VK_QK_COM_X_hi = {{ (fpstr_hi .Vk.Qk.X) }};
  uint256 private constant VK_QK_COM_Y_lo = {{ (fpstr_lo .Vk.Qk.Y) }};
  uint256 private constant VK_QK_COM_Y_hi = {{ (fpstr_hi .Vk.Qk.Y) }};
  {{ range $index, $element := .Vk.S }}
  uint256 private constant VK_S{{ inc $index }}_COM_X_lo = {{ (fpstr_lo $element.X) }};
	uint256 private constant VK_S{{ inc $index }}_COM_X_hi = {{ (fpstr_hi $element.X) }};
  uint256 private constant VK_S{{ inc $index }}_COM_Y_lo = {{ (fpstr_lo $element.Y) }};
  uint256 private constant VK_S{{ inc $index }}_COM_Y_hi = {{ (fpstr_hi $element.Y) }};
  {{ end }}
  uint256 private constant VK_COSET_SHIFT = {{ frstr .Vk.CosetShift }};
  
  {{ range $index, $element := .Vk.Qcp}}
  uint256 private constant VK_QCP_{{ $index }}_X_lo = {{ (fpstr_lo $element.X) }};
	uint256 private constant VK_QCP_{{ $index }}_X_hi = {{ (fpstr_hi $element.X) }};
  uint256 private constant VK_QCP_{{ $index }}_Y_lo = {{ (fpstr_lo $element.Y) }};
  uint256 private constant VK_QCP_{{ $index }}_Y_hi = {{ (fpstr_hi $element.Y) }};
  {{ end }}
  ...
  // offset proof
  {{ $offset := 0 }}
	uint256 private constant PROOF_L_COM_X = {{ hex $offset }};{{ $offset = add $offset 0x30}}
  uint256 private constant PROOF_L_COM_Y = {{ hex $offset }};{{ $offset = add $offset 0x30}}
  uint256 private constant PROOF_R_COM_X = {{ hex $offset }};{{ $offset = add $offset 0x30}}
  uint256 private constant PROOF_R_COM_Y = {{ hex $offset }};{{ $offset = add $offset 0x30}}
  uint256 private constant PROOF_O_COM_X = {{ hex $offset }};{{ $offset = add $offset 0x30}}
  uint256 private constant PROOF_O_COM_Y = {{ hex $offset }};{{ $offset = add $offset 0x30}}

  // h = h_0 + x^{n+2}h_1 + x^{2(n+2)}h_2
  uint256 private constant PROOF_H_0_COM_X = {{ hex $offset }};{{ $offset = add $offset 0x30}}
  uint256 private constant PROOF_H_0_COM_Y = {{ hex $offset }};{{ $offset = add $offset 0x30}}
  uint256 private constant PROOF_H_1_COM_X = {{ hex $offset }};{{ $offset = add $offset 0x30}}
  uint256 private constant PROOF_H_1_COM_Y = {{ hex $offset }};{{ $offset = add $offset 0x30}}
  uint256 private constant PROOF_H_2_COM_X = {{ hex $offset }};{{ $offset = add $offset 0x30}}
  ...
  // [Z]
  uint256 private constant PROOF_GRAND_PRODUCT_COMMITMENT_X = {{ hex $offset }};{{ $offset = add $offset 0x30}}
  uint256 private constant PROOF_GRAND_PRODUCT_COMMITMENT_Y = {{ hex $offset }};{{ $offset = add $offset 0x30}}
  ...
  uint256 private constant PROOF_BATCH_OPENING_AT_ZETA_X = {{ hex $offset }};{{ $offset = add $offset 0x30}}
  uint256 private constant PROOF_BATCH_OPENING_AT_ZETA_Y = {{ hex $offset }};{{ $offset = add $offset 0x30}}
  ...
  public returns(bool success) {
    ...
    //   // Beginning challenges -------------------------------------------------
      ...
      function derive_gamma(aproof, nb_pi, pi)->gamma_not_reduced {
        ...
        mstore(add(mPtr, {{ hex $offset }}), VK_S2_COM_X_hi) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_S2_COM_Y_hi) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_S3_COM_X_hi) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_S3_COM_Y_hi) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_QL_COM_X_hi) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_QL_COM_Y_hi) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_QR_COM_X_hi) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_QR_COM_Y_hi) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_QM_COM_X_hi) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_QM_COM_Y_hi) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_QO_COM_X_hi) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_QO_COM_Y_hi) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_QK_COM_X_hi) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_QK_COM_Y_hi) {{ $offset = add $offset 0x30}}
        {{ range $index, $element := .Vk.CommitmentConstraintIndexes}}
        mstore(add(mPtr, {{ hex $offset }}), VK_QCP_{{ $index }}_X_hi) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_QCP_{{ $index }}_Y_hi) {{ $offset = add $offset 0x30}}
        {{ end }}
        mstore(mPtr, FS_GAMMA) // "gamma"
        {{ $offset = 0x30 }}
        mstore(add(mPtr, {{ hex $offset }}), VK_S1_COM_X_lo) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_S1_COM_Y_lo) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_S2_COM_X_lo) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_S2_COM_Y_lo) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_S3_COM_X_lo) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_S3_COM_Y_lo) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_QL_COM_X_lo) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_QL_COM_Y_lo) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_QR_COM_X_lo) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_QR_COM_Y_lo) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_QM_COM_X_lo) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_QM_COM_Y_lo) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_QO_COM_X_lo) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_QO_COM_Y_lo) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_QK_COM_X_lo) {{ $offset = add $offset 0x30}}
        mstore(add(mPtr, {{ hex $offset }}), VK_QK_COM_Y_lo) {{ $offset = add $offset 0x30}}
        {{ range $index, $element := .Vk.CommitmentConstraintIndexes}}
        ...
        let size_commitments_lro_in_bytes := {{ hex (mul 0x60 3)}}
        ...
        let size := add({{ hex ( add 5 ( mul 11 96 ))}}, size_pi_in_bytes)
        {{ if (gt (len .Vk.CommitmentConstraintIndexes) 0 )}}
    ...
    function compute_gamma_kzg(aproof) {
      ...
      {{ $offset = 0 }}
      mstore(mPtr, FS_GAMMA_KZG) // "gamma" {{ $offset = add $offset 0x20 }}
      mstore(add(mPtr, {{ hex $offset }}), mload(add(state, STATE_ZETA))){{ $offset = add $offset 0x20 }}
      mcopy(add(mPtr, {{ hex $offset }}), add(state, add(STATE_LINEARISED_POLYNOMIAL, 0x10)), 0x30){{ $offset = add $offset 0x30 }}
      mcopy(add(mPtr, {{ hex $offset }}), add(state, add(STATE_LINEARISED_POLYNOMIAL, 0x50)), 0x30){{ $offset = add $offset 0x30 }}
      calldatacopy(add(mPtr, {{ hex $offset }}), add(aproof, PROOF_L_COM_X), 0x120)
      
      {{ $offset = $tmp }}
      mstore(add(mPtr, {{ hex $offset }}), mload(add(state, STATE_OPENING_LINEARISED_POLYNOMIAL_ZETA))) {{ $offset = add $offset 0x20 }}
      mstore(add(mPtr, {{ hex $offset }}), calldataload(add(aproof, PROOF_L_AT_ZETA))){{ $offset = add $offset 0x20 }}
      mstore(add(mPtr, {{ hex $offset }}), calldataload(add(aproof, PROOF_R_AT_ZETA))){{ $offset = add $offset 0x20 }}
      mstore(add(mPtr, {{ hex $offset }}), calldataload(add(aproof, PROOF_O_AT_ZETA))){{ $offset = add $offset 0x20 }}
      mstore(add(mPtr, {{ hex $offset }}), calldataload(add(aproof, PROOF_S1_AT_ZETA))){{ $offset = add $offset 0x20 }}
      ...
      mstore(add(mPtr, {{ hex $offset }}), calldataload(add(aproof, PROOF_GRAND_PRODUCT_AT_ZETA_OMEGA))){{ $offset = add $offset 0x20 }}

      let start_input := 0x1b // 00.."gamma"
      {{ $offset = add (sub $offset 0x20 ) 0x5 }}
      let check_staticcall := staticcall(gas(), SHA2, add(mPtr,start_input), {{ hex $offset }}, add(state, STATE_GAMMA_KZG), 0x20)
      if iszero(check_staticcall) {
        error_verify()
      }
    ...
    function compute_commitment_linearised_polynomial_ec(aproof, s1, coeff_z) {
      ...
      {{- $tmp = 0 }}
      {{- range .Vk.Qcp}}
      store_point_calldata(add(mPtr, {{ hex $offset }}), add(aproof, add(PROOF_BSB_COMMITMENTS, {{ hex $tmp }}))){{ $offset = add $offset 0xa0 }}{{ $tmp = add $tmp 0x60 }}
      {{- end }}
      {{- end }}
      ...
      mstore(add(mPtr, {{ hex $offset }}), coeff_z){{ $offset = add $offset 0xa0 }}
      ...
      let l_success := staticcall(gas(), BLS12_MSM_G1, mPtr, {{ hex $offset }}, add(state, STATE_LINEARISED_POLYNOMIAL), 0x80)
      if iszero(l_success){
        error_ec_op()
      }
...
// Solidity contract.
// The marshalling of the Fp points follows https://eips.ethereum.org/EIPS/eip-2537#fine-points-and-encoding-of-base-elements
// That is a Fp point is encoded as
// [ uint256 || uint256 ]
// [ lo		   ||		hi		]	, where lo, hi are in big endian. hi is on 16 bytes.
