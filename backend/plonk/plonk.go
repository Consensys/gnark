...
// TODO @gbotrel KZG setup polyCommit should not be an interface here
func Setup(sparseR1cs frontend.CompiledConstraintSystem, polynomialCommitment interface{}) (ProvingKey, VerifyingKey, error) {

	// TODO temporary ignoring polynomialCommitment parameter
	// in a production setup, KZG setup will not occur, it will just be read from a io.Reader
	// for now, re-running at each Setup
	...
	case *backend_bn254.SparseR1CS:
		...
		err := plonkbn254.Setup(_sparseR1cs, &pk, &vk, nil)
		if err != nil {
			return nil, nil, err
		}
		return &pk, &vk, nil
	...
	case *backend_bls12381.SparseR1CS:
		...
		err := plonkbls12381.Setup(_sparseR1cs, &pk, &vk, nil)
		if err != nil {
			return nil, nil, err
		}
		return &pk, &vk, nil
	...
	case *backend_bls12377.SparseR1CS:
		...
		err := plonkbls12377.Setup(_sparseR1cs, &pk, &vk, nil)
		if err != nil {
			return nil, nil, err
		}
		return &pk, &vk, nil
	...
	case *backend_bw6761.SparseR1CS:
		...
		err := plonkbw6761.Setup(_sparseR1cs, &pk, &vk, nil)
		if err != nil {
			return nil, nil, err
		}
		return &pk, &vk, nil
	...
	case *backend_bls24315.SparseR1CS:
		...
		var pk plonkbls24315.ProvingKey
		var vk plonkbls24315.VerifyingKey
		err := plonkbls24315.Setup(_sparseR1cs, &pk, &vk, nil)
		if err != nil {
			return nil, nil, err
		}
		return &pk, &vk, nil
...
func Prove(sparseR1cs frontend.CompiledConstraintSystem, pk ProvingKey, fullWitness frontend.Circuit) (Proof, error) {
	...
	case *backend_bn254.SparseR1CS:
		...
		return plonkbn254.Prove(_sparseR1cs, pk.(*plonkbn254.ProvingKey), w)
	...
	case *backend_bls12381.SparseR1CS:
		...
		return plonkbls12381.Prove(_sparseR1cs, pk.(*plonkbls12381.ProvingKey), w)
	...
	case *backend_bls12377.SparseR1CS:
		...
		return plonkbls12377.Prove(_sparseR1cs, pk.(*plonkbls12377.ProvingKey), w)
	...
	case *backend_bw6761.SparseR1CS:
		...
		return plonkbw6761.Prove(_sparseR1cs, pk.(*plonkbw6761.ProvingKey), w)
	...
	case *backend_bls24315.SparseR1CS:
		...
		return plonkbls24315.Prove(_sparseR1cs, pk.(*plonkbls24315.ProvingKey), w)
...
func Verify(proof Proof, vk VerifyingKey, publicWitness frontend.Circuit) error {
	...
	case *plonkbls24315.Proof:
		w := bls24315witness.Witness{}
		if err := w.FromPublicAssignment(publicWitness); err != nil {
			return err
		}
		return plonkbls24315.Verify(_proof, vk.(*plonkbls24315.VerifyingKey), w)
