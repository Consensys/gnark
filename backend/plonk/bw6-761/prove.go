...
func Prove(spr *cs.SparseR1CS, pk *ProvingKey, fullWitness witness.Witness, opts ...backend.ProverOption) (*Proof, error) {
	...
	go func() {
		...
		qkCompletedCanonical := make([]fr.Element, len(lqkcoef), pk.Domain[1].Cardinality)
	...
	fic := func(i int, fqk, l, r, o fr.Element, pi2QcPrime []fr.Element) fr.Element { // TODO @Tabaie make use of the fact that qCPrime is a selector: sparse and binary

		var ic, tmp fr.Element

		ic.Mul(&pk.evalStuff[i][idx_QL], &l)
		tmp.Mul(&pk.evalStuff[i][idx_QR], &r)
		ic.Add(&ic, &tmp)
		tmp.Mul(&pk.evalStuff[i][idx_QM], &l).Mul(&tmp, &r)
		ic.Add(&ic, &tmp)
		tmp.Mul(&pk.evalStuff[i][idx_QO], &o)
	...
	fo := func(i int, l, r, o, fz, fzs fr.Element) fr.Element {
		u := &pk.Domain[0].FrMultiplicativeGen
		var a, b, tmp fr.Element

		b.Mul(&beta, &pk.evalStuff[i][idx_ID])
		...
		b.Mul(&beta, &pk.evalStuff[i][idx_S1]).Add(&b, &l).Add(&b, &gamma)
		tmp.Mul(&beta, &pk.evalStuff[i][idx_S2]).Add(&tmp, &r).Add(&tmp, &gamma)
		b.Mul(&b, &tmp)
		tmp.Mul(&beta, &pk.evalStuff[i][idx_S3]).Add(&tmp, &o).Add(&tmp, &gamma)
	...
	fone := func(i int, fz fr.Element) fr.Element {
		one := fr.One()
		// c := pk.lLoneIOP.GetCoeff(i)
		one.Sub(&fz, &one).Mul(&one, &pk.evalStuff[i][idx_LONE])
		return one
	}
	...
	fm := func(i int, x ...fr.Element) fr.Element {
		a := fic(i, x[idx_QK], x[idx_L], x[idx_R], x[idx_O], x[idx_Bsb22Commitments:])
		b := fo(i, x[idx_L], x[idx_R], x[idx_O], x[idx_Z], x[idx_ZS])
		c := fone(i, x[idx_Z])
	...
	systemEvaluation, err := evaluate(fm, iop.Form{Basis: iop.LagrangeCoset, Layout: iop.BitReverse}, toEval...)
	if err != nil {
		return nil, err
	}
...
func evaluate(f expression, form iop.Form, x ...*iop.Polynomial) (*iop.Polynomial, error) {
	if len(x) == 0 {
