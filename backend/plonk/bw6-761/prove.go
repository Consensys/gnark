...
type dumbMemoryAllocator struct {
	lock   sync.Mutex
	memory []fr.Element
	offset int
}

func newDumbMemoryAllocator(size int) *dumbMemoryAllocator {
	return &dumbMemoryAllocator{
		memory: make([]fr.Element, size),
		offset: 0,
	}
}

func (a *dumbMemoryAllocator) Allocate(n int) []fr.Element {
	a.lock.Lock()
	defer a.lock.Unlock()
	if a.offset+n > len(a.memory) {
		panic("dumbMemoryAllocator out of memory")
	}
	res := a.memory[a.offset : a.offset+n]
	a.offset += n
	return res
}
