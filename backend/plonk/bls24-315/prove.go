...
func newInstance(ctx context.Context, spr *cs.SparseR1CS, pk *ProvingKey, fullWitness witness.Witness, opts *backend.ProverConfig) (*instance, error) {
	...
	buildPermutation(spr, &s.trace, nbVariables)
	ps := computePermutationPolynomials(&s.trace, s.domain0)
	s.trace.S1 = ps[0]
	s.trace.S2 = ps[1]
	s.trace.S3 = ps[2]
...
func (s *instance) initComputeNumerator() error {
	...
	s.cres = make([]fr.Element, s.domain1.Cardinality)
	s.twiddles0 = make([]fr.Element, n)
	if n == 1 {
		// edge case
		s.twiddles0[0].SetOne()
	} else {
		copy(s.twiddles0, s.domain0.Twiddles[0])
		for i := len(s.domain0.Twiddles[0]); i < len(s.twiddles0); i++ {
			s.twiddles0[i].Mul(&s.twiddles0[i-1], &s.twiddles0[1])
		}
	}

	cosetTable := s.domain0.CosetTable
	twiddles := s.domain1.Twiddles[0][:n]
...
func (s *instance) computeLinearizedPolynomial() error {
	...
	s.linearizedPolynomial = s.computeLinearizedPolynomial2(
		blzeta,
		brzeta,
		bozeta,
		s.alpha,
...
func (s *instance) computeLinearizedPolynomial2(lZeta, rZeta, oZeta, alpha, beta, gamma, zeta, zu fr.Element, qcpZeta, blindedZCanonical []fr.Element, pi2Canonical [][]fr.Element, pk *ProvingKey) []fr.Element {
	// TODO @gbotrel rename
