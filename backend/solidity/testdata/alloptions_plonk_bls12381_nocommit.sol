...
contract PlonkVerifier is IVerifier {
  ...
  uint256 private constant G2_SRS_1_X_0_lo = 4399589642848621164398204124397201296962024217886571843641405410947241256733;
	uint256 private constant G2_SRS_1_X_0_hi = 25444824644973203556577809959116237795;
  uint256 private constant G2_SRS_1_X_1_lo = 78119886150074642762750031977972118901260938658070081551416411393603413627049;
  uint256 private constant G2_SRS_1_X_1_hi = 17451246856739855855817626270359418148;
  uint256 private constant G2_SRS_1_Y_0_lo = 113267256378511525680632398695449744673439535326407587300911797645177082796863;
  uint256 private constant G2_SRS_1_Y_0_hi = 33724906070208243104250410690403851602;
  uint256 private constant G2_SRS_1_Y_1_lo = 8410616535445693128849343052209911585898052258539562465119471850283773820793;
  uint256 private constant G2_SRS_1_Y_1_hi = 19729188438973855576589327517463304665;
  ...
  uint256 private constant VK_INV_DOMAIN_SIZE = 45881390778235416669516772944662720107979233437961683094778201362446258536449;
  uint256 private constant VK_OMEGA = 23674694431658770659612952115660802947967373701506253797663184111817857449850;
  uint256 private constant VK_QL_COM_X_lo = 45007700117041269209118623091980848819926035558538977413531541440830855998752;
	uint256 private constant VK_QL_COM_X_hi = 15103247069207525285206075692058777801;
  uint256 private constant VK_QL_COM_Y_lo = 86220790586511000738071218770469103022000108348913262948425746600627275759719;
  uint256 private constant VK_QL_COM_Y_hi = 1474550774121820005685228790834879785;
  uint256 private constant VK_QR_COM_X_lo = 38416383109383145064906047902961636512488119208323156578752489506385535062589;
  uint256 private constant VK_QR_COM_X_hi = 29061828019905922144462959394220492474;
  uint256 private constant VK_QR_COM_Y_lo = 7449119692252341509082210824465941397512652556143401751756002707290395298249;
  uint256 private constant VK_QR_COM_Y_hi = 33458942742591692986749963427549315666;
  uint256 private constant VK_QM_COM_X_lo = 37661340731629440486950430541083039847489500980023826469966136051109005164993;
  uint256 private constant VK_QM_COM_X_hi = 27248587665527653238480447377691757484;
  uint256 private constant VK_QM_COM_Y_lo = 21211702011449515216981406263987038888486562149869033190039228142838763015053;
  uint256 private constant VK_QM_COM_Y_hi = 28208797835034341327048307661652627085;
  uint256 private constant VK_QO_COM_X_lo = 37661340731629440486950430541083039847489500980023826469966136051109005164993;
  uint256 private constant VK_QO_COM_X_hi = 27248587665527653238480447377691757484;
  uint256 private constant VK_QO_COM_Y_lo = 24230358862920350740071716193078689274111928612674005869969980121314337152798;
  uint256 private constant VK_QO_COM_Y_hi = 6356685710380564741740888365162798666;
  uint256 private constant VK_QK_COM_X_lo = 0;
  uint256 private constant VK_QK_COM_X_hi = 0;
  uint256 private constant VK_QK_COM_Y_lo = 0;
  uint256 private constant VK_QK_COM_Y_hi = 0;
  
  uint256 private constant VK_S1_COM_X_lo = 91499350040908507718552121309354629878529150553665277138360602979887159462187;
	uint256 private constant VK_S1_COM_X_hi = 8135698884742198763599700999488674847;
  uint256 private constant VK_S1_COM_Y_lo = 53254960033887685843995822416661111468146639963848067346253323431370345457;
  uint256 private constant VK_S1_COM_Y_hi = 6559741057619904231939431585448843790;
  
  uint256 private constant VK_S2_COM_X_lo = 13126010205620699147776095996102142441748387616793457005375931583161602770130;
	uint256 private constant VK_S2_COM_X_hi = 8306132303738914580490109694827761431;
  uint256 private constant VK_S2_COM_Y_lo = 76614928639645771062706771107641778931136130681263121404947344102332797387786;
  uint256 private constant VK_S2_COM_Y_hi = 22389766394051519571621344331102060675;
  
  uint256 private constant VK_S3_COM_X_lo = 83863299800281499954886460037310792406582566813374079124535875540185834405283;
	uint256 private constant VK_S3_COM_X_hi = 11403482808554526823484058858498092168;
  uint256 private constant VK_S3_COM_Y_lo = 47440136060586354357099404187460318155842875056935741912493811204513928853919;
  uint256 private constant VK_S3_COM_Y_hi = 8884874014681007527842481977533971918;
  ...
  uint256 private constant PROOF_BSB_COMMITMENTS = 0x420;
  ...
  public view returns(bool success) {
    ...
    assembly {
      ...
      function derive_gamma(aproof, nb_pi, pi)->gamma_not_reduced {
        ...
        mstore8(add(mPtr, 0x2c0), 0x40)

        // public inputs
        let _mPtr := add(mPtr, 0x320)
      ...
      function fold_state(aproof) {
        ...
        let l_success := staticcall(gas(), BLS12_MSM_G1, mPtr, 0x3c0, add(state, STATE_FOLDED_DIGESTS), 0x80)
        if iszero(l_success){
          error_ec_op()
        }
      ...
      function compute_gamma_kzg(aproof) {
        ...
        mstore(add(mPtr, 0x280), mload(add(state, STATE_OPENING_LINEARISED_POLYNOMIAL_ZETA))) 
        mstore(add(mPtr, 0x2a0), calldataload(add(aproof, PROOF_L_AT_ZETA)))
        mstore(add(mPtr, 0x2c0), calldataload(add(aproof, PROOF_R_AT_ZETA)))
        mstore(add(mPtr, 0x2e0), calldataload(add(aproof, PROOF_O_AT_ZETA)))
        mstore(add(mPtr, 0x300), calldataload(add(aproof, PROOF_S1_AT_ZETA)))
        mstore(add(mPtr, 0x320), calldataload(add(aproof, PROOF_S2_AT_ZETA)))mstore(add(mPtr, 0x340), calldataload(add(aproof, PROOF_GRAND_PRODUCT_AT_ZETA_OMEGA)))

        let start_input := 0x1b // 00.."gamma"
        
        let check_staticcall := staticcall(gas(), SHA2, add(mPtr,start_input), 0x345, add(state, STATE_GAMMA_KZG), 0x20)
      ...
      function compute_commitment_linearised_polynomial_ec(aproof, s1, coeff_z) {
        ...
        let l_success := staticcall(gas(), BLS12_MSM_G1, mPtr, 0x640, add(state, STATE_LINEARISED_POLYNOMIAL), 0x80)
        if iszero(l_success){
          error_ec_op()
        }
