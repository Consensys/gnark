...
func (assert *Assert) NotSolved(r1cs *backend_bw761.R1CS, solution backend.Assignments) {
	// setup
...
func (assert *Assert) Solved(r1cs *backend_bw761.R1CS, solution backend.Assignments, expectedValues map[string]fr.Element) {
	// setup
...
// CorrectExecution Verifies that the expected solution matches the solved variables
func (assert *Assert) CorrectExecution(r1cs *backend_bw761.R1CS, solution backend.Assignments, expectedValues map[string]fr.Element) {
	...
	for k, v := range expectedValues {
		val, ok := res[k]
		assert.True(ok, "Variable to test <"+k+"> (backend_bw761) is not tagged")
		assert.True(val.Equal(&v), "Tagged variable <"+k+"> (backend_bw761) does not have the expected value\nexpected: "+v.String()+"\ngot:\t  "+val.String())
	}
}
