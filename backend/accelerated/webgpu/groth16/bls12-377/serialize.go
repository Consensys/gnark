...
func packG1AffineJacobianBatch(points []bls12377.G1Affine) []byte {
...
func packG2AffineJacobianBatch(points []bls12377.G2Affine) []byte {
...
func decodeG1AffineFromPacked(packed []byte, err error) (bls12377.G1Affine, error) {
	if err != nil {
		return bls12377.G1Affine{}, err
	}
	if len(packed) != 2*g1CoordinateBytes {
		return bls12377.G1Affine{}, fmt.Errorf("webgpu groth16 bls12_377: expected %d G1 bytes, got %d", 2*g1CoordinateBytes, len(packed))
	}
...
func decodeG2AffineFromPacked(packed []byte, err error) (bls12377.G2Affine, error) {
	if err != nil {
		return bls12377.G2Affine{}, err
	}
	if len(packed) != 4*g2ComponentBytes {
		return bls12377.G2Affine{}, fmt.Errorf("webgpu groth16 bls12_377: expected %d G2 bytes, got %d", 4*g2ComponentBytes, len(packed))
	}
