...
const (
	frBytes           = fr.Bytes
	g1CoordinateBytes = bn254fp.Bytes
...
func readFrRegularLE(src []byte) (fr.Element, error) {
	if len(src) != frBytes {
		return fr.Element{}, fmt.Errorf("webgpu plonk bn254: expected %d Fr bytes, got %d", frBytes, len(src))
	}
	var le [frBytes]byte
	copy(le[:], src)
...
func unpackFrVectorRegularLEInto(dst []fr.Element, src []byte) error {
	if len(src) != len(dst)*frBytes {
		return fmt.Errorf("webgpu plonk bn254: expected %d Fr vector bytes, got %d", len(dst)*frBytes, len(src))
	}
	for i := range dst {
...
func canonicalizePolynomialsRegularWithWebGPU(polys []*iop.Polynomial, elementCount int) error {
	...
	for key, group := range groups {
		...
		canonicalPacked, err := bridge.Bridge.CanonicalizeQuotientVectors("bn254", valuesPacked, len(group), n, key.inputBitReversed, key.inverseCoset)
		if err != nil {
			return err
		}
...
func lagrangePolynomialsRegularWithWebGPU(polys []*iop.Polynomial, elementCount int) error {
	...
	lagrangePacked, err := bridge.Bridge.LagrangeQuotientVectors("bn254", valuesPacked, len(filtered), n)
	if err != nil {
		return err
	}
...
func decodeG1AffineFromPacked(packed []byte, err error) (curve.G1Affine, error) {
	...
	if len(packed) != 2*g1CoordinateBytes {
		return curve.G1Affine{}, fmt.Errorf("webgpu plonk bn254: expected %d G1 bytes, got %d", 2*g1CoordinateBytes, len(packed))
	}
...
func readFPMontLE(src []byte) bn254fp.Element {
	var words [4]uint64
	for i := range words {
...
func writeFPMontLE(dst []byte, value *bn254fp.Element) {
