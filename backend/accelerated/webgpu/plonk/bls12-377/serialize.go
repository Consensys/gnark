...
func packFrVectorsRegularLEPaddedInto(dst []byte, vectors [][]fr.Element, elementCount int) ([]byte, error) {
	if elementCount <= 0 {
		return nil, errors.New("webgpu plonk bls12_377: empty MSM batch")
	}
	...
	for i, values := range vectors {
		if len(values) > elementCount {
			return nil, fmt.Errorf("webgpu plonk bls12_377: MSM batch vector %d has %d elements, expected at most %d", i, len(values), elementCount)
		}
...
func readFrRegularLE(src []byte) (fr.Element, error) {
	if len(src) != frBytes {
		return fr.Element{}, fmt.Errorf("webgpu plonk bls12_377: expected %d Fr bytes, got %d", frBytes, len(src))
	}
	var le [frBytes]byte
	copy(le[:], src)
...
func unpackFrVectorRegularLEInto(dst []fr.Element, src []byte) error {
	if len(src) != len(dst)*frBytes {
		return fmt.Errorf("webgpu plonk bls12_377: expected %d Fr vector bytes, got %d", len(dst)*frBytes, len(src))
	}
	for i := range dst {
...
func canonicalizePolynomialsRegularWithWebGPU(polys []*iop.Polynomial, elementCount int) error {
	...
	for _, p := range polys {
		...
		if len(coeffs) != n {
			return fmt.Errorf("webgpu plonk bls12_377: canonicalize polynomial has %d coefficients, expected %d", len(coeffs), n)
		}
		switch p.Basis {
		case iop.Lagrange:
		case iop.LagrangeCoset:
		default:
			return fmt.Errorf("webgpu plonk bls12_377: unsupported polynomial basis %d", p.Basis)
		}
		switch p.Layout {
		case iop.Regular:
		case iop.BitReverse:
		default:
			return fmt.Errorf("webgpu plonk bls12_377: unsupported polynomial layout %d", p.Layout)
		}
	...
	for key, group := range groups {
		...
		canonicalPacked, err := bridge.Bridge.CanonicalizeQuotientVectors("bls12_377", valuesPacked, len(group), n, key.inputBitReversed, key.inverseCoset)
		if err != nil {
			return err
		}
		if len(canonicalPacked) != len(valuesPacked) {
			return fmt.Errorf("webgpu plonk bls12_377: quotient canonicalize returned %d bytes, expected %d", len(canonicalPacked), len(valuesPacked))
		}
		for i, p := range group {
...
func lagrangePolynomialsRegularWithWebGPU(polys []*iop.Polynomial, elementCount int) error {
	...
	for _, p := range polys {
		...
		if p.Basis != iop.Canonical || p.Layout != iop.Regular {
			return fmt.Errorf("webgpu plonk bls12_377: expected canonical regular polynomial, got basis %d layout %d", p.Basis, p.Layout)
		}
		if len(p.Coefficients()) != n {
			return fmt.Errorf("webgpu plonk bls12_377: lagrange polynomial has %d coefficients, expected %d", len(p.Coefficients()), n)
		}
	...
	lagrangePacked, err := bridge.Bridge.LagrangeQuotientVectors("bls12_377", valuesPacked, len(filtered), n)
	if err != nil {
		return err
	}
	if len(lagrangePacked) != len(valuesPacked) {
		return fmt.Errorf("webgpu plonk bls12_377: quotient lagrange returned %d bytes, expected %d", len(lagrangePacked), len(valuesPacked))
	}
	for i, p := range filtered {
...
func decodeG1AffineFromPacked(packed []byte, err error) (curve.G1Affine, error) {
	...
	if len(packed) != 2*g1CoordinateBytes {
		return curve.G1Affine{}, fmt.Errorf("webgpu plonk bls12_377: expected %d G1 bytes, got %d", 2*g1CoordinateBytes, len(packed))
	}
...
func decodeG1AffineBatchFromPacked(packed []byte, count int, err error) ([]curve.G1Affine, error) {
	...
	if len(packed) != expected {
		return nil, fmt.Errorf("webgpu plonk bls12_377: expected %d G1 batch bytes, got %d", expected, len(packed))
	}
