...
func (s *instance) computeNumerator() (*iop.Polynomial, error) {
	...
	if commitmentCount < 0 {
		return nil, fmt.Errorf("webgpu plonk bls12_377: quotient evaluator expected at least %d dynamic vectors, got %d", quotientBaseDynamicVectorCount, len(quotientDynamicPolyIDs))
	}
	...
	for i, id := range quotientDynamicPolyIDs {
		if id >= len(s.x) || s.x[id] == nil {
			return nil, fmt.Errorf("webgpu plonk bls12_377: missing quotient dynamic polynomial %d", id)
		}
		coeffs := s.x[id].Coefficients()
		if len(coeffs) != int(n) {
			return nil, fmt.Errorf("webgpu plonk bls12_377: quotient dynamic polynomial %d has %d coefficients, expected %d", id, len(coeffs), n)
		}
	...
	if len(outputPacked) != rho*vectorBytes {
		return nil, fmt.Errorf("webgpu plonk bls12_377: quotient all-coset evaluator returned %d bytes, expected %d", len(outputPacked), rho*vectorBytes)
	}
