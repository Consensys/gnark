...
func buildQuotientAuxCache(domain0, domain1 *fft.Domain) (quotientAuxCache, error) {
	...
	rho := int(domain1.Cardinality / domain0.Cardinality)
	if n <= 0 || rho <= 0 {
		return quotientAuxCache{}, fmt.Errorf("webgpu plonk bls12_381: invalid quotient auxiliary domain n=%d rho=%d", n, rho)
	}
...
func (pk *ProvingKey) preloadQuotientStaticCaches(trace *native.Trace, domain0, domain1 *fft.Domain) error {
	...
	if staticCache == nil {
		return errors.New("webgpu plonk bls12_381: missing static numerator cache")
	}
	n := int(domain0.Cardinality)
	rho := int(domain1.Cardinality / domain0.Cardinality)
	if len(staticCache.cosets) != rho {
		return fmt.Errorf("webgpu plonk bls12_381: static numerator cache has %d cosets, expected %d", len(staticCache.cosets), rho)
	}
	quotientAux := staticCache.quotientAux
	if !quotientAux.valid(n, rho) {
		return errors.New("webgpu plonk bls12_381: invalid quotient auxiliary cache")
	}
	auxMontCacheKey := staticCache.webgpuAuxMontKey
	if auxMontCacheKey <= 0 || int(uint32(auxMontCacheKey)) != auxMontCacheKey {
		return fmt.Errorf("webgpu plonk bls12_381: invalid quotient auxiliary WebGPU cache key %d", auxMontCacheKey)
	}
...
func (pk *ProvingKey) quotientStaticBridgeInputs(rho, staticVectorCount, commitmentCount, n, vectorBytes int) (quotientStaticBridgeInputs, error) {
	...
	quotientAux := staticCache.quotientAux
	if !quotientAux.valid(n, rho) {
		return quotientStaticBridgeInputs{}, errors.New("webgpu plonk bls12_381: invalid quotient auxiliary cache")
	}
	staticMontCacheKeysPacked, err := packStaticMontCacheKeys(staticCache, rho)
	if err != nil {
	...
	auxMontCacheKey := staticCache.webgpuAuxMontKey
	if auxMontCacheKey <= 0 || int(uint32(auxMontCacheKey)) != auxMontCacheKey {
		return quotientStaticBridgeInputs{}, fmt.Errorf("webgpu plonk bls12_381: invalid quotient auxiliary WebGPU cache key %d", auxMontCacheKey)
	}
...
func packStaticMontCacheKeys(staticCache *staticNumeratorCache, rho int) ([]byte, error) {
	if len(staticCache.webgpuStaticMontKeys) != rho || len(staticCache.webgpuStaticMontPopulated) != rho {
		return nil, errors.New("webgpu plonk bls12_381: invalid static numerator WebGPU cache metadata")
	}
	out := make([]byte, rho*4)
	for i := 0; i < rho; i++ {
		key := staticCache.webgpuStaticMontKeys[i]
		if key <= 0 || int(uint32(key)) != key {
			return nil, fmt.Errorf("webgpu plonk bls12_381: invalid static numerator WebGPU cache key %d", key)
		}
...
func packStaticNumeratorPolys(dst []byte, polys staticNumeratorPolys, staticVectorCount, commitmentCount, n, vectorBytes int) error {
	if len(polys.qcp) != commitmentCount {
		return fmt.Errorf("webgpu plonk bls12_381: quotient evaluator expected %d qcp vectors, got %d", commitmentCount, len(polys.qcp))
	}
	staticVectors := polys.polynomials()
	if len(staticVectors) != staticVectorCount {
		return fmt.Errorf("webgpu plonk bls12_381: quotient evaluator expected %d static vectors, got %d", staticVectorCount, len(staticVectors))
	}
	for i, p := range staticVectors {
		if p == nil {
			return fmt.Errorf("webgpu plonk bls12_381: missing quotient static polynomial %d", i)
		}
		coeffs := p.Coefficients()
		if len(coeffs) != n {
			return fmt.Errorf("webgpu plonk bls12_381: quotient static polynomial %d has %d coefficients, expected %d", i, len(coeffs), n)
		}
