...
func (pk *ProvingKey) PrepareWithCS(spr *cs.SparseR1CS) error {
	...
	if err := bridge.Bridge.PrewarmQuotientTransformDomain("bls12_381", int(domain0.Cardinality)); err != nil {
		return err
	}
	if err := bridge.Bridge.PrewarmQuotientEvaluateKernel("bls12_381", len(trace.Qcp)); err != nil {
		return err
	}
	return bridge.Bridge.PrewarmQuotientCanonicalizeDomain("bls12_381", int(domain1.Cardinality))
}
