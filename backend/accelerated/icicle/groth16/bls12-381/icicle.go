...
func (pk *ProvingKey) setupDevicePointers(device *icicle_runtime.Device) error {
	...
	icicle_runtime.RunOnDevice(device, func(args ...any) {
		...
		if err := icicle_bls12381.AffineFromMontgomery(pk.G1Device.A); err != icicle_runtime.Success {
	...
	icicle_runtime.RunOnDevice(device, func(args ...any) {
		...
		if err := icicle_bls12381.AffineFromMontgomery(pk.G1Device.B); err != icicle_runtime.Success {
	...
	icicle_runtime.RunOnDevice(device, func(args ...any) {
		...
		if err := icicle_bls12381.AffineFromMontgomery(pk.G1Device.K); err != icicle_runtime.Success {
	...
	icicle_runtime.RunOnDevice(device, func(args ...any) {
		...
		err := icicle_bls12381.AffineFromMontgomery(pk.G1Device.Z)
...
func Prove(r1cs *cs.R1CS, pk *ProvingKey, fullWitness witness.Witness, opts ...backend.ProverOption) (*groth16_bls12381.Proof, error) {
	...
	if opt.Accelerator != "icicle" {
		return groth16_bls12381.Prove(r1cs, &pk.ProvingKey, fullWitness, opts...)
	}
	...
	solverOpts = append(solverOpts, solver.OverrideHint(bsb22ID, func(_ *big.Int, in []*big.Int, out []*big.Int) error {
		...
		proofCommitmentIcicle := make(icicle_core.HostSlice[icicle_bls12381.Projective], 1)
	...
	if numCommitmentKeys > 0 {
		...
		poksIcicle := make([]icicle_core.HostSlice[icicle_bls12381.Projective], numCommitmentKeys)
		for i := range poksIcicle {
			poksIcicle[i] = make(icicle_core.HostSlice[icicle_bls12381.Projective], 1)
		}
	...
	computeKRS := func() error {
		...
		cfg := icicle_msm.GetDefaultMSMConfig()
		resKrs2 := make(icicle_core.HostSlice[icicle_bls12381.Projective], 1)
		start := time.Now()
		...
		resKrs := make(icicle_core.HostSlice[icicle_bls12381.Projective], 1)
		cfg.AreScalarsMontgomeryForm = true
		start = time.Now()
