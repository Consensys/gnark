...
func (s *instance) commitToEntangledPolyAndBlinding(p, b *iop.Polynomial, id int) (commit curve.G1Affine, err error) {
	...
	// LAGRANGE
	p.ToCanonical(s.domain0).ToRegular()
	commit, err = kzg.Commit(p.Coefficients(), s.pk.KzgSplitted[id])
...
func (s *instance) computeQuotient() (err error) {
	...
	// commit to h
	if err := s.commitToQuotient(s.h1(), s.h2(), s.h3(), s.proof); err != nil {
		return err
	}
...
func (s *instance) commitToQuotient(h1, h2, h3 []fr.Element, proof *Proof) error {
	g := new(errgroup.Group)
	...
	g.Go(func() (err error) {
		commitmentH1, err = kzg.Commit(h1, s.pk.KzgSplitted[rh1])
		return
	})

	g.Go(func() (err error) {
		commitmentH2, err = kzg.Commit(h2, s.pk.KzgSplitted[rh2])
		return
	})

	g.Go(func() (err error) {
		commitmentH3, err = kzg.Commit(h3, s.pk.KzgSplitted[rh3])
		return
	})
