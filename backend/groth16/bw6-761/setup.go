...
func Setup(r1cs *cs.R1CS, pk *ProvingKey, vk *VerifyingKey) error {
	...
	*/
	...
	cI := make([]int, len(r1cs.CommitmentInfo)) // number of private committed wires seen so far for each commitment
	...
	for i := range A {
		...
		if isPublic = i < r1cs.GetNbPublicVariables(); !isPublic {
			...
			for j := range r1cs.CommitmentInfo { // does commitment j commit to i?
