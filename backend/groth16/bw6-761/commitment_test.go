...
import (
	...
	bw6_761groth16 "github.com/consensys/gnark/backend/groth16/bw6-761"
...
func setup(t *testing.T, circuit frontend.Circuit) (*cs.R1CS, *bw6_761groth16.ProvingKey, *bw6_761groth16.VerifyingKey) {
	_ccs, err := frontend.Compile(ecc.BW6_761.ScalarField(), r1cs.NewBuilder, circuit)
	...
	pk := new(bw6_761groth16.ProvingKey)
	vk := new(bw6_761groth16.VerifyingKey)
	err = bw6_761groth16.Setup(ccs, pk, vk)
	assert.NoError(t, err)

	return ccs, pk, vk
}

func prove(t *testing.T, assignment frontend.Circuit, ccs *cs.R1CS, pk *bw6_761groth16.ProvingKey) (fr.Vector, *bw6_761groth16.Proof) {
	_witness, err := frontend.NewWitness(assignment, ecc.BW6_761.ScalarField())
	assert.NoError(t, err)

	proof, err := bw6_761groth16.Prove(ccs, pk, _witness)
	assert.NoError(t, err)
...
func test(t *testing.T, circuit frontend.Circuit, assignment frontend.Circuit) {
	...
	assert.NoError(t, bw6_761groth16.Verify(proof, vk, public))
}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			ok := assert.NotPanics(t, func() {
				err = bw6_761groth16.Verify(proof, vk, public)
