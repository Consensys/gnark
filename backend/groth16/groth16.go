...
// ProverOpt type storing flags useful for the prover
type ProverOpt uint8

// Safe the prover checks if there is an error before launching the multi exp
// Unsafe the prover runs the multi exponentiation wether or not the r1cs is satisfied
const (
	Safe ProverOpt = iota
	Unsafe
)
...
// opts provide options Unsafe or Safe for the prover. In the case where Unsafe is provided,
...
func Prove(r1cs r1cs.R1CS, pk ProvingKey, solution interface{}, opts ...ProverOpt) (Proof, error) {
	...
	case *backend_bls377.R1CS:
		return groth16_bls377.Prove(_r1cs, pk.(*groth16_bls377.ProvingKey), _solution, opt[0])
	case *backend_bls381.R1CS:
		return groth16_bls381.Prove(_r1cs, pk.(*groth16_bls381.ProvingKey), _solution, opt[0])
	case *backend_bn256.R1CS:
		return groth16_bn256.Prove(_r1cs, pk.(*groth16_bn256.ProvingKey), _solution, opt[0])
	case *backend_bw761.R1CS:
		return groth16_bw761.Prove(_r1cs, pk.(*groth16_bw761.ProvingKey), _solution, opt[0])
	default:
		panic("unrecognized R1CS curve type")
	}
}
