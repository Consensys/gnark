	...
	add := func(_i1, _i2 interface{}) *Constraint {
		...
		case *Constraint:
			...
			case *Constraint:
				return cs.add(c1, c2)
			default:
				return cs.addConstant(c1, elementFromInterface(c2))
			}
		default:
			switch c2 := _i2.(type) {
			case *Constraint:
				return cs.addConstant(c2, elementFromInterface(c1))
			default:
				panic("invalid type")
			}
		}
	...
	mul := func(_i1, _i2 interface{}) *Constraint {
		...
		case *Constraint:
			...
			case *Constraint:
				return cs.mul(c1, c2)
			default:
				return cs.mulConstant(c1, elementFromInterface(c2))
			}
		default: // i1 is not a Constraint type, so c2 must be
			switch c2 := _i2.(type) {
			case *Constraint:
				return cs.mulConstant(c2, elementFromInterface(c1))
			default:
				panic("invalid type")
			}
...
func (cs *CS) MUSTBE_LESS_OR_EQ(c *Constraint, input interface{}) {
	// parse input
	constant := elementFromInterface(input)
...
func (cs *CS) SELECT(b *Constraint, i1, i2 interface{}) *Constraint {
	...
	default:
		c1Fr := elementFromInterface(i1)
		c2Fr := elementFromInterface(i2)
		c1Fr.Sub(&c1Fr, &c2Fr)
		expression := linearExpression{
			term{Wire: b.outputWire, Coeff: c1Fr, Operation: mul},
			term{Wire: cs.Constraints[0].outputWire, Coeff: curve.One(), Operation: mul},
		}
		return newConstraint(cs, &expression)
	}
}
...
// Element converts a [uint64, int, string, *Element] to a curve.Element
// see internal/curve
// this methods panic if the provided interface is not supported
func Element(i1 interface{}) curve.Element {
	return elementFromInterface(i1)
}

func elementFromInterface(i1 interface{}) curve.Element {
