...
// if x and y are not of type cs.Constraint
// they must be curve.Element and will be allocated (ALLOCATE) in the circuit
func NewPoint(circuit *cs.CS, _x, _y interface{}) Point {
...
func (p *Point) IsOnCurve(ecurve twistededwards.CurveParams) {
	...
	circuit := p.circuit
	var one curve.Element
	one.SetOne()
	l1 := cs.LinearCombination{cs.Term{Constraint: p.X, Coeff: ecurve.A}}
	l2 := cs.LinearCombination{cs.Term{Constraint: p.X, Coeff: one}}
	axx := circuit.MUL(l1, l2)

	yy := circuit.MUL(p.Y, p.Y)
	lhs := circuit.ADD(axx, yy)
	l1 = cs.LinearCombination{cs.Term{Constraint: p.X, Coeff: ecurve.D}}
	l2 = cs.LinearCombination{cs.Term{Constraint: p.X, Coeff: one}}
...
func (p *Point) Add(p1 *Point, base *twistededwards.Point, ecurve twistededwards.CurveParams) *Point {
	...
	// constraint 2
	den := cs.LinearCombination{
		cs.Term{Constraint: oneWire, Coeff: one},
		cs.Term{Constraint: b, Coeff: duv},
	}
	num := cs.LinearCombination{
		cs.Term{Constraint: p1.X, Coeff: base.Y},
		cs.Term{Constraint: p1.Y, Coeff: base.X},
	}
	...
	num = cs.LinearCombination{
		cs.Term{Constraint: p1.Y, Coeff: base.Y},
		cs.Term{Constraint: p1.X, Coeff: tmp},
	}
...
func (p *Point) AddGeneric(p1, p2 *Point, ecurve twistededwards.CurveParams) *Point {
	...
	num := cs.LinearCombination{
		cs.Term{Constraint: beta, Coeff: one},
		cs.Term{Constraint: gamma, Coeff: one},
	}
	den := cs.LinearCombination{
		cs.Term{Constraint: oneWire, Coeff: one},
		cs.Term{Constraint: tau, Coeff: ecurve.D},
	}
	res.X = circuit.DIV(num, den)
	var minusa curve.Element
	minusa.Neg(&ecurve.A)
	num = cs.LinearCombination{
		cs.Term{Constraint: delta, Coeff: one},
		cs.Term{Constraint: epsilon, Coeff: minusa},
	}
	var minusd curve.Element
	minusd.Neg(&ecurve.D)
	den = cs.LinearCombination{
		cs.Term{Constraint: oneWire, Coeff: one},
		cs.Term{Constraint: tau, Coeff: minusd},
	}
	res.Y = circuit.DIV(num, den)
...
func (p *Point) ScalarMul(p1 interface{}, ecurve twistededwards.CurveParams, scalar *cs.Constraint, n int) *Point {
...
func (p *Point) scalarMulFixedBase(p1 *twistededwards.Point, ecurve twistededwards.CurveParams, scalar *cs.Constraint, n int) *Point {
...
func (p *Point) scalarMulNonFixedBase(p1 *Point, scalar *cs.Constraint, c twistededwards.CurveParams, n int) *Point {
