...
func (s *Scheme) Commit(p polynomial.Polynomial) polynomial.Digest {
	res := &MockDigest{Digest: p.(bw761.Poly)}
	return res
}

// Open computes an opening proof of _p at _val.
// Returns a MockProof, which is an empty interface.
func (s *Scheme) Open(_val interface{}, _p polynomial.Polynomial) polynomial.OpeningProof { //Open(p *bw761.Poly, val *fr.Element) *MockProof {
	return &MockProof{}
}
...
func (s *Scheme) BatchOpenSinglePoint(_val, challenge interface{}, _p ...polynomial.Polynomial) polynomial.BatchOpeningProofSinglePoint { //Open(p *bw761.Poly, val *fr.Element) *MockProof {
	return &MockProof{}
}
