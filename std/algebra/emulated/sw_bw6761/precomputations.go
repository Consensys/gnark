...
var precomputedLines [4][189]baseEl
var precomputedLinesOnce sync.Once

func getPrecomputedLines() [4][189]baseEl {
...
func computePrecomputedLines() [4][189]baseEl {
	var PrecomputedLines [4][189]baseEl
	_, _, _, G2AffGen := bw6761.Generators()
	lines := bw6761.PrecomputeLines(G2AffGen)
	for j := 0; j < 189; j++ {
		PrecomputedLines[0][j] = emulated.ValueOf[emulated.BW6761Fp](lines[0][j].R0)
		PrecomputedLines[1][j] = emulated.ValueOf[emulated.BW6761Fp](lines[0][j].R1)
		PrecomputedLines[2][j] = emulated.ValueOf[emulated.BW6761Fp](lines[1][j].R0)
		PrecomputedLines[3][j] = emulated.ValueOf[emulated.BW6761Fp](lines[1][j].R1)
	}
