...
func DecomposeScalarG1(mod *big.Int, inputs []*big.Int, outputs []*big.Int) error {
	return UnwrapHint(inputs, outputs, func(field *big.Int, inputs, outputs []*big.Int) error {
		...
		for outputs[0].Cmp(lambda) < 1 && outputs[1].Cmp(lambda) < 1 {
			outputs[0].Add(outputs[0], lambda)
			outputs[0].Add(outputs[0], one)
			outputs[1].Add(outputs[1], lambda)
		}
...
func (c *Curve[B, S]) ScalarMulGLV(Q *AffinePoint[B], s *emulated.Element[S], opts ...algopts.AlgebraOption) *AffinePoint[B] {
	...
	sd, err := c.scalarApi.NewHint(DecomposeScalarG1, 3, s)
	if err != nil {
	...
	// first bit
	B1.X = tableQ[0].X
	B1.Y = *c.baseApi.Select(s1bits[nbits-1], &tableQ[1].Y, &tableQ[0].Y)
	Acc = *c.doubleAndAdd(&Acc, &B1)
	B1.X = tablePhiQ[0].X
	B1.Y = *c.baseApi.Select(s2bits[nbits-1], &tablePhiQ[1].Y, &tablePhiQ[0].Y)
	Acc = *c.Add(&Acc, &B1)

	// second bit
	B1.X = tableQ[0].X
	B1.Y = *c.baseApi.Select(s1bits[nbits-2], &tableQ[1].Y, &tableQ[0].Y)
	Acc = *c.doubleAndAdd(&Acc, &B1)
	B1.X = tablePhiQ[0].X
	B1.Y = *c.baseApi.Select(s2bits[nbits-2], &tablePhiQ[1].Y, &tablePhiQ[0].Y)
	Acc = *c.Add(&Acc, &B1)
	...
	for i := nbits - 3; i > 0; i-- {
		...
		B2.Y = *c.baseApi.Select(s2bits[i], &tablePhiQ[1].Y, &tablePhiQ[0].Y)
		B1 = *c.Add(&B1, &B2)
		Acc = *c.doubleAndAdd(&Acc, &B1)
	}
...
func (c *Curve[B, S]) JointScalarMulGLV(Q, R *AffinePoint[B], s, t *emulated.Element[S], opts ...algopts.AlgebraOption) *AffinePoint[B] {
	...
	td, err := c.scalarApi.NewHint(DecomposeScalarG1, 3, t)
	if err != nil {
		// err is non-nil only for invalid number of inputs
		panic(err)
	}
	...
	tablePhiS[0].X = *c.baseApi.Mul(&tableS[0].X, &thirdRootOne1)
	tablePhiS[0].Y = tableS[0].Y
	tablePhiS[1].X = *c.baseApi.Mul(&tableS[1].X, &thirdRootOne1)
	tablePhiS[1].Y = tableS[1].Y
	tablePhiS[2].X = *c.baseApi.Mul(&tableS[2].X, &thirdRootOne1)
	tablePhiS[2].Y = tableS[2].Y
	tablePhiS[3].X = *c.baseApi.Mul(&tableS[3].X, &thirdRootOne1)
	tablePhiS[3].Y = tableS[3].Y
