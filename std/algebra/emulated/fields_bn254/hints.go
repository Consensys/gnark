...
func finalExpHint(nativeMod *big.Int, nativeInputs, nativeOutputs []*big.Int) error {
	return emulated.UnwrapHint(nativeInputs, nativeOutputs,
		func(mod *big.Int, inputs, outputs []*big.Int) error {
			var tmp, x3, w, x, f, c, cInv, un, root bn254.E12
			var s, exponent, rInv, mInv big.Int

			f.C0.B0.A0.SetBigInt(inputs[0])
			f.C0.B0.A1.SetBigInt(inputs[1])
			f.C0.B1.A0.SetBigInt(inputs[2])
			f.C0.B1.A1.SetBigInt(inputs[3])
			f.C0.B2.A0.SetBigInt(inputs[4])
			f.C0.B2.A1.SetBigInt(inputs[5])
			f.C1.B0.A0.SetBigInt(inputs[6])
			f.C1.B0.A1.SetBigInt(inputs[7])
			f.C1.B1.A0.SetBigInt(inputs[8])
			f.C1.B1.A1.SetBigInt(inputs[9])
			f.C1.B2.A0.SetBigInt(inputs[10])
			f.C1.B2.A1.SetBigInt(inputs[11])

			// exponent = (p**12 - 1) / 3
			exponent.SetString("", 10)
			root.C0.B2.A0.SetString("8204864362109909869166472767738877274689483185363591877943943203703805152849")
			root.C0.B2.A1.SetString("17912368812864921115467448876996876278487602260484145953989158612875588124088")

			if un.Exp(f, &exponent).IsOne() {
				w.SetOne()
				c.Set(&f)
			} else if un.Exp(*f.Mul(&f, &root), &exponent).IsOne() {
				w.Set(&root)
				c.Set(&f)
			} else {
				w.Square(&root)
			...
			// c.CubeRoot(&c)
			cInv.Inverse(&c)
			s.SetString("", 10)
			x.Exp(c, &s)
			x3.Square(&x).Mul(&x3, &x).Mul(&x3, &cInv)
			t := 0
			for !x3.IsOne() {
				t++
				tmp.Square(&x3)
			...
			w.C0.B0.A0.BigInt(outputs[12])
			w.C0.B0.A1.BigInt(outputs[13])
			w.C0.B1.A0.BigInt(outputs[14])
			w.C0.B1.A1.BigInt(outputs[15])
			w.C0.B2.A0.BigInt(outputs[16])
			w.C0.B2.A1.BigInt(outputs[17])
			w.C1.B0.A0.BigInt(outputs[18])
			w.C1.B0.A1.BigInt(outputs[19])
			w.C1.B1.A0.BigInt(outputs[20])
			w.C1.B1.A1.BigInt(outputs[21])
			w.C1.B2.A0.BigInt(outputs[22])
			w.C1.B2.A1.BigInt(outputs[23])
