...
func (circuit *g1JointScalarMulEdgeCases) Define(api frontend.API) error {
	...
	expected4 := G1Affine{}
	infinity := G1Affine{X: 0, Y: 0}
	expected1.jointScalarMul(api, infinity, infinity, circuit.R, circuit.S, algopts.WithUseSafe())
	expected2.jointScalarMul(api, circuit.A, circuit.B, big.NewInt(0), big.NewInt(0), algopts.WithUseSafe())
	expected3.jointScalarMul(api, circuit.A, infinity, circuit.R, circuit.S, algopts.WithUseSafe())
	expected4.jointScalarMul(api, circuit.A, circuit.B, circuit.R, big.NewInt(0), algopts.WithUseSafe())
	_expected := G1Affine{}
	_expected.ScalarMul(api, circuit.A, circuit.R, algopts.WithUseSafe())
	expected1.AssertIsEqual(api, infinity)
	expected2.AssertIsEqual(api, infinity)
