...
var precomputedLines [8][32]fields_bls24315.E2
var precomputedLinesOnce sync.Once

func getPrecomputedLines() [8][32]fields_bls24315.E2 {
...
func computePrecomputedLines() [8][32]fields_bls24315.E2 {
	var PrecomputedLines [8][32]fields_bls24315.E2
	_, _, _, G2AffGen := bls24315.Generators()
	lines := bls24315.PrecomputeLines(G2AffGen)
	for j := 0; j < 32; j++ {
		PrecomputedLines[0][j].Assign(&lines[0][j].R0.B0)
		PrecomputedLines[1][j].Assign(&lines[0][j].R0.B1)
		PrecomputedLines[2][j].Assign(&lines[0][j].R1.B0)
		PrecomputedLines[3][j].Assign(&lines[0][j].R1.B1)
		PrecomputedLines[4][j].Assign(&lines[1][j].R0.B0)
		PrecomputedLines[5][j].Assign(&lines[1][j].R0.B1)
		PrecomputedLines[6][j].Assign(&lines[1][j].R1.B0)
		PrecomputedLines[7][j].Assign(&lines[1][j].R1.B1)
