...
func finalExpHint(_ *big.Int, inputs, outputs []*big.Int) error {
	...
	var root, rootPthInverse, root2304thInverse, residueWitness, scalingFactor bls12377.E12
	var exponent, exponentInv, finalExpFactor, polyFactor big.Int
	// polyFactor = (1-x)/192
	polyFactor.SetString("49927723505680384", 10)
	// finalExpFactor = ((q^12 - 1) / r) / (2304 * polyFactor)
	...
	exponent.Mul(&finalExpFactor, big.NewInt(2304))
	...
	// 2. get 2304th-root inverse
	exponent.Mul(&finalExpFactor, &polyFactor)
	root.Exp(millerLoop, &exponent)
	if root.IsOne() {
		root2304thInverse.SetOne()
	} else {
		exponentInv.ModInverse(&exponent, big.NewInt(2304))
		exponent.Neg(&exponentInv).Mod(&exponent, big.NewInt(2304))
		root2304thInverse.Exp(root, &exponent)
	}

	// 3. shift the Miller loop result so that millerLoop * scalingFactor
	// is of order finalExpFactor
	scalingFactor.Mul(&rootPthInverse, &root2304thInverse)
