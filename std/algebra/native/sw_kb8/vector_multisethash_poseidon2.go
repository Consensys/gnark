...
var errPoseidon2MismatchedHalves = errors.New("kb8 Poseidon2 multiset hash: msgsLow and msgsHigh must have the same length")
...
type Poseidon2Accumulator struct {
	curve *Curve
	sums  [maptocurve_kb8.PqN]G1Affine
}
...
func (a *Poseidon2Accumulator) Insert(msgLow, msgHigh frontend.Variable) error {
	pts, err := maptocurve_kb8.MapPoseidon2(a.curve.api, msgLow, msgHigh)
	if err != nil {
		return err
	}
	for i := range a.sums {
...
func (a *Poseidon2Accumulator) Digest() [maptocurve_kb8.PqN]G1Affine {
	return a.sums
}
...
func (c *Curve) HashPoseidon2(msgsLow, msgsHigh []frontend.Variable) ([maptocurve_kb8.PqN]G1Affine, error) {
	if len(msgsLow) != len(msgsHigh) {
		return [maptocurve_kb8.PqN]G1Affine{}, errPoseidon2MismatchedHalves
	}
	acc := NewPoseidon2Accumulator(c)
	for i := range msgsLow {
		if err := acc.Insert(msgsLow[i], msgsHigh[i]); err != nil {
			return [maptocurve_kb8.PqN]G1Affine{}, err
		}
	}
