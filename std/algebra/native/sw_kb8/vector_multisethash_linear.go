...
type LinearAccumulator struct {
	curve *Curve
	sums  [maptocurve_kb8.LinearN]G1Affine
}
...
func (a *LinearAccumulator) Insert(msg frontend.Variable) error {
	pts, err := maptocurve_kb8.MapLinear(a.curve.api, msg)
	if err != nil {
		return err
	}
	for i := range a.sums {
...
func (a *LinearAccumulator) Digest() [maptocurve_kb8.LinearN]G1Affine {
	return a.sums
}
...
func (c *Curve) HashLinear(msgs []frontend.Variable) ([maptocurve_kb8.LinearN]G1Affine, error) {
	acc := NewLinearAccumulator(c)
	for _, msg := range msgs {
		if err := acc.Insert(msg); err != nil {
			return [maptocurve_kb8.LinearN]G1Affine{}, err
		}
	}
