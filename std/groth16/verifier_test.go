...
func (circuit *mimcCircuit) Define(curveID ecc.ID, gnark frontend.API) error {
	mimc, err := mimc.NewMiMC("seed", curveID, gnark)
	if err != nil {
		return err
	}
	...
	result := mimc.Sum()
	gnark.AssertIsEqual(result, circuit.Hash)
	return nil
}
...
func (circuit *verifierCircuit) Define(curveID ecc.ID, gnark frontend.API) error {
	...
	Verify(gnark, pairingInfo, circuit.InnerVk, circuit.InnerProof, []frontend.Variable{circuit.Hash})
