...
// either [frontend.Variable] or [fieldextension.ExtensionVariable]. The
...
func (mc *mulCheck[T]) check(api frontend.API, peval, coef frontend.Variable) {
	...
	if mc.f.extensionApi == nil {
		...
		aext := mc.a.evaluation.(fieldextension.ExtensionVariable)
		bext := mc.b.evaluation.(fieldextension.ExtensionVariable)
		ls := mc.f.extensionApi.Mul(aext, bext)

		rext := mc.r.evaluation.(fieldextension.ExtensionVariable)
		pevalext := peval.(fieldextension.ExtensionVariable)
		cext := mc.c.evaluation.(fieldextension.ExtensionVariable)
		kext := mc.k.evaluation.(fieldextension.ExtensionVariable)
...
func (f *Field[T]) evalWithChallengeExtension(a *Element[T], at []frontend.Variable) frontend.Variable {
	...
	// type assert it.
	atext := make([]fieldextension.ExtensionVariable, len(at))
	for i := 0; i < len(at); i++ {
		atexti, ok := at[i].(fieldextension.ExtensionVariable)
		if !ok {
...
func (f *Field[T]) performDeferredChecks(api frontend.API) error {
	...
	if f.extensionApi == nil {
		...
		multicommit.WithWideCommitment(api, func(api frontend.API, commitment []frontend.Variable) error {
			...
			at := make([]fieldextension.ExtensionVariable, coefsLen)
			at[0] = commitment
			for i := 1; i < len(at); i++ {
				atexti := at[i-1]
				at[i] = f.extensionApi.Mul((fieldextension.ExtensionVariable)(atexti), commitment)
			}
...
func (mc *mvCheck[T]) check(api frontend.API, peval, coef frontend.Variable) {
	...
	if mc.f.extensionApi == nil {
		...
		for i, term := range mc.mv.Terms {
			...
			for i, pow := range term {
				for j := 0; j < pow; j++ {
					valsexti := mc.vals[i].evaluation.(fieldextension.ExtensionVariable)
