...
func bn254ProveHint(data *circuitData, ins []*big.Int, outs []*big.Int) error {
	if len(ins) != 0 {
		return fmt.Errorf("the prove hint takes no input")
	}
	typed := data.typed.(bn254CircuitData)

	proof, err := gkr.Prove(typed.circuit, typed.assignments, fiatshamir.WithHash(mimc.NewMiMC()), gkr.WithPool(&typed.memoryPool)) // TODO: Do transcriptSettings properly
	if err != nil {
		return err
	}
