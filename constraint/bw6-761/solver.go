...
func newSolver(cs *system, witness fr.Vector, opt *csolver.Config) (*solver, error) {
	// check witness size
	witnessOffset := 0
...
func (s *solver) printLogs(logs []constraint.LogEntry) {
	...
	for i := 0; i < len(logs); i++ {
		...
		s.logger.Debug("api.Println", slog.String(slog.SourceKey, logs[i].Caller), slog.String("debug", logLine))
	}
}
