...
func (le *GeneralLazyInputs) FetchLazy(r1cs constraint.R1CS, j int) constraint.R1C {
	...
	if j < len(le.inputConstraints) {
		le.shift = le.inputConstraints[j].L[0].WireID() - staticR1cs[j].L[0].WireID()
...
func createGeneralLazyInputsFunc(key string) func(inputs []constraint.R1C, loc int, constraintsNum int, paramsNum int) constraint.LazyInputs {
	return func(inputs []constraint.R1C, loc int, constraintsNum int, paramsNum int) constraint.LazyInputs {
