...
type LazyInputsFactory func(constraints []R1C, loc int, constraintsNum int, paramsNum int) LazyInputs
...
func NewLazyInputs(key string, inputConstraints []R1C, loc int, constraintsNum int, paramsNum int) LazyInputs {
	if factory, exists := lazyInputsFactoryMap[key]; exists {
		return factory(inputConstraints, loc, constraintsNum, paramsNum)
	} else {
