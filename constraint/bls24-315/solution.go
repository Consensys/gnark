...
func (s *solution) accumulateInto(t constraint.Term, r *fr.Element) {
	...
	vID := t.WireID()
	{
		// TODO @gbotrel perf check, this is needed only when evaluating logs & debug info
		// (or maybe also with plonk for the K constant?)
		if t.IsConstant() {
...
func (s *solution) logValue(log constraint.LogEntry) string {
	...
	for j := 0; j < len(log.ToResolve); j++ {
		...
		// if vID == 0 { // TODO @gbotrel too R1CS will break on SparseR1CS
		// 	// it's just a constant
		// 	if cID == constraint.CoeffIdMinusOne {
		...
		// 	continue
		// }
		// if !(cID == constraint.CoeffIdMinusOne || cID == constraint.CoeffIdOne) {
		// 	toResolve = append(toResolve, s.coefficients[cID].String())
		// }
		// if !s.solved[vID] {
		// 	toResolve = append(toResolve, unsolvedVariable)
