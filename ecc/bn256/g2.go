...
func (p *G2Jac) MultiExpNew(curve *Curve, points []G2Affine, scalars []fr.Element) chan G2Jac {
	...
	// create buckets
	var buckets [32][255]lockedG2Jac
	for i := 0; i < 32; i++ {
		for j := 0; j < 255; j++ {
	...
	go func() {
		...
		for i := 0; i < 32; i++ {
			...
			for j := 0; j < 255; j++ {
				acc.Add(curve, &buckets[i][254-j].G2Jac)
...
func (p *G2Jac) MultiExpNewNoPool(curve *Curve, points []G2Affine, scalars []fr.Element) *G2Jac {
	...
	work := func(chunk int) G2Jac {

		buckets := make([][255]G2Jac, nbCpus)
		for i := 0; i < nbCpus; i++ {
			for j := 0; j < 255; j++ {
				buckets[i][j].Set(&curve.g2Infinity)
			}
