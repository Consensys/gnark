...
func (builder *builder[E]) Commit(v ...frontend.Variable) (frontend.Variable, error) {
	...
	// privateCommittedSeeker.Seek returns an index into the original, unsliced
	// commitment list, but existingCommitmentIndexes gets progressively sliced
	// down below as we walk t.VID in increasing order. Keep an unsliced copy to
	// look the committer index up against.
	allCommitmentIndexes := existingCommitmentIndexes
