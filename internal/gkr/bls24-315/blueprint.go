...
func (b *BlueprintProve) Solve(s constraint.Solver[constraint.U64], inst constraint.Instruction) error {
	...
	hsh := hash.NewHash(b.HashName + "_BLS24_315")
	fsSettings := fiatshamir.WithHash(hsh, insBytes...)

	// Call the BLS24_315-specific Prove function (assignments already WireAssignment type)
	proof, err := Prove(solveBlueprint.Circuit, assignments, fsSettings)
	if err != nil {
		return fmt.Errorf("bls24_315 prove failed: %w", err)
	}
...
// NewBlueprints creates and registers all GKR blueprints for BLS24_315
