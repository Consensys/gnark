...
func (a WireAssignment) Complete(wires gkrtypes.Wires) WireAssignment {
	...
	insPtr := make([]*fr.Element, maxNbIns)
...
func (stack *elementStack) newElement() *fr.Element {
	stack.nbUsed++
	if stack.nbUsed >= len(stack.allocated) {
		stack.allocated = append(stack.allocated, new(fr.Element))
	}
	return stack.allocated[api.nbUsed-1]
}

func (stack *elementStack) push(x *fr.Element) {
	stack.newElement().Set(x)
}

func opAdd(stack *elementStack, x []*fr.Element) *fr.Element {
...
func opMulAcc(stack *elementStack, x []*fr.Element) *fr.Element {
	// result = x[0] + (x[1] * x[2])
...
func opNeg(stack *elementStack, x []*fr.Element) *fr.Element {
	res := stack.newElement()
	res.Neg(x[0])
	return res
}

func opSub(stack *elementStack, x []*fr.Element) *fr.Element {
	res := stack.newElement()
	res.Sub(x[0], x[1])
	for i := 2; i < len(x); i++ {
		res.Sub(res, x[i])
...
func opMul(stack *elementStack, x []*fr.Element) *fr.Element {
	res := stack.newElement()
	res.Mul(x[0], x[1])
	for i := 2; i < len(x); i++ {
		res.Mul(res, x[i])
...
func opSumExp17(stack *elementStack, x []*fr.Element) *fr.Element {
...
func executeCompiledGate(gate *gkrtypes.CompiledGate, stack *elementStack, inputs []*fr.Element) *fr.Element {

	// Instruction executor functions
	executors := [...]func(*elementStack, []*fr.Element) *fr.Element{
	...
	for i := range gate.Instructions {
		...
		operands := make([]*fr.Element, len(inst.Inputs))
...
func newGateEvaluator(gate *gkrtypes.CompiledGate) *gateEvaluator {
	eval := &gateEvaluator{
		gate:     gate,
		nbConsts: gate.NbConstants(),
		inputs:   make([]*fr.Element, 0, gate.NbInputs),
	}
...
func (e *gateEvaluator) evaluate() *fr.Element {
