...
type zeroCheckClaims struct {
	...
	strides      []small_rational.SmallRational // per-group Horner stride (foldingCoeff^nbSources)
		...
		for h := start; h < end; h++ {
			...
			for d := range degree {
				...
				for g, nbW := range c.nbGroupWires {
					...
					var groupSum small_rational.SmallRational
...
func (r *resources) proveSumcheckLevel(levelI int) sumcheckProof {
	...
	claims.strides = make([]small_rational.SmallRational, len(groups))
