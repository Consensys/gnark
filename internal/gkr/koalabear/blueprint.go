...
func init() {
	...
	constraint.RegisterGkrBlueprintTypes(ecc.UNKNOWN, BlueprintSolve{}, BlueprintProve{}, BlueprintGetAssignment{})
}
...
// BlueprintSolve is a KOALABEAR-specific blueprint for solving GKR circuit instances.
...
var _ constraint.BlueprintStateful[constraint.U32] = (*BlueprintSolve)(nil)
...
func (b *BlueprintSolve) Solve(s constraint.Solver[constraint.U32], inst constraint.Instruction) error {
	...
	if len(b.assignments[0]) <= instanceI {
		...
		for wI := range b.assignments {
			for len(b.assignments[wI]) <= instanceI {
				b.assignments[wI] = append(b.assignments[wI], koalabear.Element{})
			}
		}
	}
	...
	for wI := range b.Circuit {
		...
		if w.IsInput() {
			...
			// Copy directly from constraint.U32 to fr.Element (both in Montgomery form)
...
// BlueprintProve is a KOALABEAR-specific blueprint for generating GKR proofs.
...
var _ constraint.BlueprintSolvable[constraint.U32] = (*BlueprintProve)(nil)

// Solve implements the BlueprintSolvable interface for proving.
func (b *BlueprintProve) Solve(s constraint.Solver[constraint.U32], inst constraint.Instruction) error {
	b.lock.Lock()
	defer b.lock.Unlock()
	...
	// Get assignments from solve blueprint (already in koalabear.Element form)
	...
	for len(calldata) != 0 {
		...
		// Copy directly from constraint.U32 to fr.Element (both in Montgomery form)
		var challenge koalabear.Element
		copy(challenge[:], val[:])
		insBytes = append(insBytes, challenge.Marshal())
	}

	// Create Fiat-Shamir settings
	hsh := hash.NewHash(b.HashName + "_KOALABEAR")
	fsSettings := fiatshamir.WithHash(hsh, insBytes...)

	// Call the KOALABEAR-specific Prove function (assignments already WireAssignment type)
	proof, err := Prove(solveBlueprint.Circuit, assignments, fsSettings)
	if err != nil {
		return fmt.Errorf("koalabear prove failed: %w", err)
	}
...
// BlueprintGetAssignment is a KOALABEAR-specific blueprint for retrieving wire assignments.
...
var _ constraint.BlueprintSolvable[constraint.U32] = (*BlueprintGetAssignment)(nil)

// Solve implements the BlueprintSolvable interface for getting assignments.
func (b *BlueprintGetAssignment) Solve(s constraint.Solver[constraint.U32], inst constraint.Instruction) error {
	b.lock.Lock()
	defer b.lock.Unlock()
...
// NewBlueprints creates and registers all GKR blueprints for KOALABEAR
