...
type eqTimesGateEvalSumcheckLazyClaims struct {
	...
	evaluationPoints   [][]koalabear.Element // xᵢ: the points at which the prover has made claims about the evaluation of w
	claimedEvaluations []koalabear.Element   // yᵢ = w(xᵢ), allegedly
...
func (e *eqTimesGateEvalSumcheckLazyClaims) combinedSum(a koalabear.Element) koalabear.Element {
...
func (e *eqTimesGateEvalSumcheckLazyClaims) verifyFinalEval(r []koalabear.Element, combinationCoeff, purportedValue koalabear.Element, uniqueInputEvaluations []koalabear.Element) error {
	// the eq terms ( E )
	...
	// the w(...) term
	var gateEvaluation koalabear.Element
...
type eqTimesGateEvalSumcheckClaims struct {
	wireI              int                   // the wire for which we are making the claim, with value w
	evaluationPoints   [][]koalabear.Element // xᵢ: the points at which the prover has made claims about the evaluation of w
	claimedEvaluations []koalabear.Element   // yᵢ = w(xᵢ)
...
func (c *eqTimesGateEvalSumcheckClaims) combine(combinationCoeff koalabear.Element) polynomial.Polynomial {
	varsNum := c.varsNum()
	eqLength := 1 << varsNum
...
// m <- eq(q, -).
// e <- e + m
func (c *eqTimesGateEvalSumcheckClaims) eqAcc(e, m polynomial.MultiLin, q []koalabear.Element) {
	n := len(q)
...
func (c *eqTimesGateEvalSumcheckClaims) computeGJ() polynomial.Polynomial {
	...
	gJ := make([]koalabear.Element, degGJ)
	var mu sync.Mutex
	computeAll := func(start, end int) { // compute method to allow parallelization across instances
		...
		res := make([]koalabear.Element, degGJ)
		...
		mlEvals := make([]koalabear.Element, degGJ*len(ml))
...
func (c *eqTimesGateEvalSumcheckClaims) next(challenge koalabear.Element) polynomial.Polynomial {
...
func (c *eqTimesGateEvalSumcheckClaims) proveFinalEval(r []koalabear.Element) []koalabear.Element {
	...
	evaluations := make([]koalabear.Element, len(injection))
...
func newClaimsManager(wires []*Wire, assignment WireAssignment, o settings) (manager claimsManager) {
	...
	for i, wire := range wires {
		...
		manager.claims[i] = &eqTimesGateEvalSumcheckLazyClaims{
			wireI:              i,
			evaluationPoints:   make([][]koalabear.Element, 0, wire.NbClaims()),
...
func (m *claimsManager) add(wire int, evaluationPoint []koalabear.Element, evaluation koalabear.Element) {
	claim := m.claims[wire]
...
func getChallenges(transcript *fiatshamir.Transcript, names []string) ([]koalabear.Element, error) {
	res := make([]koalabear.Element, len(names))
...
func Prove(c Circuit, assignment WireAssignment, transcriptSettings fiatshamir.Settings, options ...Option) (Proof, error) {
	...
	var firstChallenge []koalabear.Element
	...
	for i := len(c) - 1; i >= 0; i-- {
		...
		if wire.NoProof() { // input wires with one claim only
			proof[i] = sumcheckProof{
				...
				finalEvalProof:  []koalabear.Element{},
			}
		} else {
...
func iterateElems(elems []koalabear.Element, counter *int, yield func(int, *koalabear.Element) bool) bool {
	for i := range elems {
...
func (p Proof) flatten() iter.Seq2[int, *koalabear.Element] {
	return func(yield func(int, *koalabear.Element) bool) {
		var counter int
		for i := range p {
...
func newGateEvaluator(gate *gkrtypes.GateBytecode, nbIn int, elementPool ...*polynomial.Pool) gateEvaluator {
	...
	if len(elementPool) > 0 {
		...
		e.vars = make([]koalabear.Element, gate.NbConstants()+nbIn+len(gate.Instructions))
	}
...
func (e *gateEvaluator) pushInput(input *koalabear.Element) {
...
func (e *gateEvaluator) evaluate(top ...koalabear.Element) *koalabear.Element {
	for i := range top {
		e.pushInput(&top[i])
	}
