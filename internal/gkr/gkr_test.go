...
func TestSingleAddGateTwoInstances(t *testing.T) {
	testSingleAddGate(t, []small_rational.SmallRational{four, three}, []small_rational.SmallRational{two, three})
}
...
func TestSumcheckFromSingleInputTwoIdentityGatesGateTwoInstances(t *testing.T) {
	...
	assignment := WireAssignment{&circuit[0]: []small_rational.SmallRational{two, three}}
	var o settings
...
func testSingleAddGate(t *testing.T, inputAssignments ...[]small_rational.SmallRational) {
	c := make(Circuit, 3)
	c[2] = Wire{
		Gate:   GetGate(Add2),
		Inputs: []*Wire{&c[0], &c[1]},
...
func setRandomSlice(slice []small_rational.SmallRational) {
	for i := range slice {
