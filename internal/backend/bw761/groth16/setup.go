...
func Setup(r1cs *bw761backend.R1CS, pk *ProvingKey, vk *VerifyingKey) {
	...
	*/
	...
	domain := bw761backend.NewDomain(nbConstraints)
...
func setupABC(r1cs *bw761backend.R1CS, g *bw761backend.Domain, toxicWaste toxicWaste) (A []fr.Element, B []fr.Element, C []fr.Element) {
	...
	for _, c := range r1cs.Constraints {
		...
		for _, t := range c.L {
			bw761backend.MulAdd(t, r1cs, &tmp, &ithLagrangePolt, &A[t.ConstraintID()])
		}
		for _, t := range c.R {
			bw761backend.MulAdd(t, r1cs, &tmp, &ithLagrangePolt, &B[t.ConstraintID()])
		}
		for _, t := range c.O {
			bw761backend.MulAdd(t, r1cs, &tmp, &ithLagrangePolt, &C[t.ConstraintID()])
		}
...
func DummySetup(r1cs *bw761backend.R1CS, pk *ProvingKey) {
	// get R1CS nb constraints, wires and public/private inputs
	nbWires := r1cs.NbWires
	nbConstraints := r1cs.NbConstraints
