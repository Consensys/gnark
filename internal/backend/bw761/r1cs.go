...
// GetCurveID returns curve ID as defined in gurvy (gurvy.BW761)
...
func (r1cs *R1CS) solveR1C(r *r1c.R1C, wireInstantiated []bool, wireValues []fr.Element) {
	...
	case r1c.BinaryDec:
		...
		// log of c>=1 where c is a power of 2
		quickLog := func(bi big.Int) int {
			words := bi.Bits()
			res := 64 * (len(words) - 1) // TODO assumes uint is 64 bits
			msw := words[len(words)-1]
			for msw != 0 { // always >0 since c>0
				msw = msw >> 1
				res++
			}
			res--
