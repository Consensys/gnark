...
func Prove(r1cs *bn256backend.R1CS, pk *ProvingKey, solution map[string]interface{}) (*Proof, error) {
	...
	// fft domain (computeH)
	fftDomain := bn256backend.NewDomain(r1cs.NbConstraints)
...
func computeH(a, b, c []fr.Element, fftDomain *bn256backend.Domain) []fr.Element {
	...
	go func() {
		...
		bn256backend.BitReverse(expTable)
		m.Unlock()
	}()
	...
	FFTa := func(s []fr.Element) {
		// FFT inverse
		bn256backend.FFT(s, fftDomain.GeneratorInv, bn256backend.DIF)
		...
		// FFT coset
		bn256backend.FFT(s, fftDomain.Generator, bn256backend.DIT)
		wg.Done()
	}
	wg.Add(3)
	go FFTa(a)
	...
	// ifft_coset
	bn256backend.FFT(a, fftDomain.GeneratorInv, bn256backend.DIF)
