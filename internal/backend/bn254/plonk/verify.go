...
func Verify(proof *Proof, vk *VerifyingKey, publicWitness bn254witness.Witness) error {
	...
	// linearizedpolynomial + pi(ζ) + α*(Z(μζ))*(l(ζ)+s1(ζ)+γ)*(r(ζ)+s2(ζ)+γ)*(o(ζ)+γ) - α²*L₁(ζ)
	...
	_s1.Add(&l, &s1).Add(&_s1, &gamma) // (l(ζ)+s1(ζ)+γ)
	_s2.Add(&r, &s2).Add(&_s2, &gamma) // (r(ζ)+s2(ζ)+γ)
	_o.Add(&o, &gamma)                 // (o(ζ)+γ)

	_s1.Mul(&_s1, &_s2).
		Mul(&_s1, &_o).
		Mul(&_s1, &alpha).
		Mul(&_s1, &zu) //  α*(Z(μζ))*(l(ζ)+s1(ζ)+γ)*(r(ζ)+s2(ζ)+γ)*(o(ζ)+γ)
	...
	// Compute H(ζ) using the previous result: H(ζ) = prev_result/(ζⁿ⁻¹)
