...
func setupABC(r1cs *bls381backend.R1CS, g *bls381backend.Domain, toxicWaste toxicWaste) (A []fr.Element, B []fr.Element, C []fr.Element) {
	...
	for _, c := range r1cs.Constraints {
		...
		for _, t := range c.L {
			bls381backend.MulAdd(t, r1cs, &tmp, &ithLagrangePolt, &A[t.ConstraintID()])
		}
		for _, t := range c.R {
			bls381backend.MulAdd(t, r1cs, &tmp, &ithLagrangePolt, &B[t.ConstraintID()])
		}
		for _, t := range c.O {
			bls381backend.MulAdd(t, r1cs, &tmp, &ithLagrangePolt, &C[t.ConstraintID()])
		}
