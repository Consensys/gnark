...
func TestSerialization(t *testing.T) {
	...
	for name := range circuits.Circuits {
		t.Run(name, func(t *testing.T) {
			...
			r1cs1, err := frontend.Compile(ecc.BLS24_317, r1cs.NewBuilder, tc.Circuit)
			if err != nil {
				t.Fatal(err)
			}
			...
			r1cs2, err := frontend.Compile(ecc.BLS24_317, r1cs.NewBuilder, tc.Circuit)
			if err != nil {
				t.Fatal(err)
			}
...
func BenchmarkSolve(b *testing.B) {
	...
	var c circuit
	ccs, err := frontend.Compile(ecc.BLS24_317, r1cs.NewBuilder, &c)
	if err != nil {
		b.Fatal(err)
	}
	...
	w.X = 1
	w.Y = 1
	witness, err := frontend.NewWitness(&w, ecc.BLS24_317)
	if err != nil {
		b.Fatal(err)
	}
