...
type SparseR1CS struct {
	compiled.SparseR1CS[fr.Element, *fr.Element]
...
func NewSparseR1CS(ccs compiled.SparseR1CS[fr.Element, *fr.Element], coefficients []*big.Int) *SparseR1CS {
	cs := SparseR1CS{
		SparseR1CS:   ccs,
...
func (cs *SparseR1CS) computeHints(c compiled.SparseR1C[fr.Element, *fr.Element], solution *solution) (int, error) {
	r := -1
	lID, rID, oID := c.L.WireID(), c.R.WireID(), c.O.WireID()

	if (!c.L.IsZero() || !c.M[0].IsZero()) && !solution.solved[lID] {
	...
	if (!c.R.IsZero() || !c.M[1].IsZero()) && !solution.solved[rID] {
		// check if it's a hint
	...
	if (!c.O.IsZero()) && !solution.solved[oID] {
		// check if it's a hint
...
func (cs *SparseR1CS) solveConstraint(c compiled.SparseR1C[fr.Element, *fr.Element], solution *solution, coefficientsNegInv []fr.Element) error {
	...
	if lro == 1 { // we solve for R: u1L+u2R+u3LR+u4O+k=0 => R(u2+u3L)+u1L+u4O+k = 0
		...
		u3.Mul(&c.M[0].Coeff, &c.M[1].Coeff)
		u1.Set(&c.L.Coeff)
		u2.Set(&c.R.Coeff)
	...
	// o = - ((m0 * m1) + l + r + c.K) / c.O
	o.Mul(&m0, &m1).Add(&o, &l).Add(&o, &r).Add(&o, &c.K)
	o.Div(&o, &cID)
	o.Neg(&o)
...
func (cs *SparseR1CS) IsSolved(witness *witness.Witness, opts ...backend.ProverOption) error {
	...
	v := witness.Vector.(*bls24_317witness.Witness)
...
// r[3] = qM⋅(xaxb)
// r[4] = qC
func (cs *SparseR1CS) formatConstraint(c compiled.SparseR1C[fr.Element, *fr.Element]) (r [5]string) {
	isZeroM := (c.M[0].IsZero()) && (c.M[1].IsZero())
	...
	if isZeroM {
		...
		sbb.Reset()
		sbb.WriteString(c.M[0].Coeff.String())
		sbb.WriteString("⋅")
		sbb.WriteByte('(')
...
func (cs *SparseR1CS) termToString(t compiled.Term[fr.Element, *fr.Element], sbb *strings.Builder, vOnly bool) {
	if !vOnly {
		if t.IsOne() {
...
func (cs *SparseR1CS) checkConstraint(c compiled.SparseR1C[fr.Element, *fr.Element], solution *solution) error {
	l := solution.computeTerm(c.L)
	r := solution.computeTerm(c.R)
	m0 := solution.computeTerm(c.M[0])
	m1 := solution.computeTerm(c.M[1])
	...
	// l + r + (m0 * m1) + o + c.K == 0
	var t fr.Element
	t.Mul(&m0, &m1).Add(&t, &l).Add(&t, &r).Add(&t, &o).Add(&t, &c.K)
	if !t.IsZero() {
...
// CurveID returns curve ID as defined in gnark-crypto (ecc.BLS24-317)
