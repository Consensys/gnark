...
import (
	...
	"bytes"
	bls24_317groth16 "github.com/consensys/gnark/internal/backend/bls24-317/groth16"
	"reflect"
	"testing"
...
func referenceCircuit() (constraint.ConstraintSystem, frontend.Circuit) {
...
func BenchmarkSetup(b *testing.B) {
	...
	var pk bls24_317groth16.ProvingKey
	var vk bls24_317groth16.VerifyingKey
	b.ResetTimer()

	b.Run("setup", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			bls24_317groth16.Setup(r1cs.(*cs.R1CS), &pk, &vk)
		}
	})
}
...
func BenchmarkProver(b *testing.B) {
	...
	fullWitness := bls24_317witness.Witness{}
	...
	var pk bls24_317groth16.ProvingKey
	bls24_317groth16.DummySetup(r1cs.(*cs.R1CS), &pk)

	b.ResetTimer()
	b.Run("prover", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			_, _ = bls24_317groth16.Prove(r1cs.(*cs.R1CS), &pk, fullWitness, backend.ProverConfig{})
		}
	})
}
...
func BenchmarkVerifier(b *testing.B) {
	...
	publicWitness := bls24_317witness.Witness{}
	...
	var vk bls24_317groth16.VerifyingKey
	bls24_317groth16.Setup(r1cs.(*cs.R1CS), &pk, &vk)
	proof, err := bls24_317groth16.Prove(r1cs.(*cs.R1CS), &pk, fullWitness, backend.ProverConfig{})
	if err != nil {
		panic(err)
	}
	...
	b.Run("verifier", func(b *testing.B) {
		for i := 0; i < b.N; i++ {
			_ = bls24_317groth16.Verify(proof, &vk, publicWitness)
		}
	})
}
...
func BenchmarkProofSerialization(b *testing.B) {
	...
	// ---------------------------------------------------------------------------------------------
	// bls24_317groth16.Proof binary serialization
	b.Run("proof: binary serialization (bls24_317groth16.Proof)", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
	...
	b.Run("proof: binary deserialization (bls24_317groth16.Proof)", func(b *testing.B) {
		var buf bytes.Buffer
		_, _ = proof.WriteTo(&buf)
		var proofReconstructed bls24_317groth16.Proof
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
	...
	// ---------------------------------------------------------------------------------------------
	// bls24_317groth16.Proof binary serialization (uncompressed)
	b.Run("proof: binary raw serialization (bls24_317groth16.Proof)", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			var buf bytes.Buffer
	...
	b.Run("proof: binary raw deserialization (bls24_317groth16.Proof)", func(b *testing.B) {
		var buf bytes.Buffer
		_, _ = proof.WriteRawTo(&buf)
		var proofReconstructed bls24_317groth16.Proof
		b.ResetTimer()
